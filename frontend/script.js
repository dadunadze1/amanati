...
function completeLogin(payload) {
  ...
  state.currentDayKey = getTodayKey();
  hideModal(els.setupModal);
  hideModal(els.authModal);
...
async function handleAction(action, value, sourceElement) {
  ...
  const handlers = {
    ...
    adminMapToggleCourier: () => adminMapToggleCourier(value),
    ...
    setStatus: () => {
      const status = sourceElement.dataset.status;
      if (!state.isAdmin && status === "delivered") {
        return confirmCourierDelivered(value);
      }
      return updatePinStatus(value, status);
    },
    logout,
  };
...
async function renderCourierStatsProfileDialog() {
  ...
  const body = `
    <div class="courier-stats-profile-panel">
      ...
      ${renderCourierCalendar(records, range.start)}
...
renderAdminMapPanel = function () {
  const pins = state.activePins;
  const couriers = state.adminMapCouriers;
  const visiblePins = filterPinsForAdminMap(pins);
  const body = `
    <div class="admin-map-panel-modern">
      ${renderAdminMapSummary(pins, visiblePins)}
      ${renderAdminMapStatusFilters()}
      ${renderAdminMapQuickActions()}
      ${renderAdminMapCourierList(couriers, pins)}
    </div>
  `;
  const actions = [{ label: "დახურვა", variant: "secondary", action: closeDialog }];
...
bindAdminMapPanelEvents = function () {};

applyAdminMapFilters = function () {
  clearAdminMapPins();
  const visiblePins = filterPinsForAdminMap(state.activePins);
  renderParcelMarkers(visiblePins);
  const summary = document.getElementById("adminMapSummary");
...
filterPinsForAdminMap = function (pins = state.activePins) {
  if (!state.isAdmin) return pins;

  const filters = getAdminMapFilters();
  const selected = new Set((filters.selectedCouriers || []).map(normalizeUsername));
  const includeAll = Boolean(filters.includeAllCouriers);
  const status = filters.status || "all";

  return (pins || []).filter((pin) => {
    const pinStatus = pin.status || "pending";

    if (status !== "all" && pinStatus !== status) return false;

    if (!pin.courierUsername) return false;
...
renderAdminMapSummary = function (pins, visiblePins, innerOnly = false) {
  const html = `
    ${renderAdminMapSummaryCard("სულ პინები", pins.length)}
    ${renderAdminMapSummaryCard("ნაჩვენები", visiblePins.length)}
    ${renderAdminMapSummaryCard("პროცესში", visiblePins.filter((pin) => (pin.status || "pending") === "pending").length)}
  ...
  `;
  return innerOnly ? html : `<div id="adminMapSummary" class="admin-map-summary-grid">${html}</div>`;
};

function renderAdminMapSummaryCard(label, value, actionLabel = "", actionName = "") {
  return `
    <div class="admin-map-summary-item">
      <span>${escapeHtml(label)}</span>
      <strong>${escapeHtml(value)}</strong>
      ${actionLabel && actionName ? `<button class="admin-map-summary-action" type="button" data-action="${escapeAttr(actionName)}">${escapeHtml(actionLabel)}</button>` : ""}
    </div>
  `;
}

function renderAdminMapStatusFilters() {
  const { status } = getAdminMapFilters();
  const statuses = [
    ["all", "ყველა"],
    ["pending", "პროცესში"],
    ["delivered", "ჩაბარებული"],
    ["failed", "ვერ ჩაბარებული"],
  ];

  return `
    <div class="admin-map-filter-grid" aria-label="სტატუსის ფილტრი">
      ${statuses.map(([value, label]) => `
        <button class="admin-map-filter-card ${status === value ? "is-active" : ""}" type="button" data-action="adminMapSetStatus" data-value="${escapeAttr(value)}">
...
function renderAdminMapQuickActions() {
  return `
    <div class="admin-map-quick-actions">
      <button class="admin-map-summary-action admin-map-quick-action" type="button" data-action="adminMapShowAllPins">ყველა პინი</button>
    </div>
  `;
}

renderAdminMapCourierList = function (couriers, pins) {
  const filters = getAdminMapFilters();
  const selected = new Set(filters.selectedCouriers.map(normalizeUsername));

  const courierCards = couriers.map((courier) => {
    const stats = getAdminMapCourierStats(courier, pins);
    const isActive = !filters.includeAllCouriers && selected.has(normalizeUsername(courier.username));
    return `
      <button class="admin-map-courier-card ${isActive ? "is-active" : ""}" type="button" data-action="adminMapToggleCourier" data-value="${escapeAttr(courier.username)}">
        <span class="admin-map-courier-main">
          <strong>${escapeHtml(userDisplayName(courier))}</strong>
          <small>${escapeHtml(courier.phone || courier.username || "")}</small>
        </span>
  ...
  return `
    <div class="admin-map-courier-grid">
...
function getAdminMapPinStats(pins) {
  const list = Array.isArray(pins) ? pins : [];
  return {
    total: list.length,
    pending: list.filter((pin) => (pin.status || "pending") === "pending").length,
    delivered: list.filter((pin) => pin.status === "delivered").length,
    failed: list.filter((pin) => pin.status === "failed").length,
  };
}

getAdminMapCourierStats = function (courier, pins) {
  return getAdminMapPinStats((pins || []).filter((pin) => normalizeUsername(pin.courierUsername) === normalizeUsername(courier.username)));
};
...
function adminMapToggleCourier(username) {
  ...
  setAdminMapFilters({
    ...
    selectedCouriers: state.adminMapCouriers
      .map((courier) => courier.username)
      .filter((courierUsername) => selected.has(normalizeUsername(courierUsername))),
  });
  refreshAdminMapPanel();
}
...
showUnassignedAdminPins = function () {
  setAdminMapFilters({
    includeAllCouriers: false,
    selectedCouriers: [],
    showUnassigned: true,
...
async function openFinanceAdmin() {
  ...
  const records = [...pins, ...history].filter((parcel) => parcelMatchesStatsDateRange(parcel, range.start, range.end));
  const delivered = getDeliveredParcels(records).length;
  const adminProfit = sumAdminProfit(records);
  const body = `
    <div class="finance-panel">
      ${renderDateRangeToolbar({
      ...
      <section class="finance-summary-grid">
        <div class="finance-summary-item"><span>არჩეული პერიოდი</span><strong>${escapeHtml(formatDateRangeLabel(range.start, range.end))}</strong></div>
        <div class="finance-summary-item"><span>ადმინის მოგება</span><strong>${escapeHtml(formatMoney(adminProfit))}</strong></div>
        <div class="finance-summary-item"><span>ჩაბარებული</span><strong>${delivered}</strong></div>
      </section>
    </div>
  `;
...
async function calculateStats(username, sinceDate) {
  ...
  const records = allRecords.filter((pin) => new Date(pin.completedAt || pin.archivedAt || pin.createdAt) >= sinceDate);
...
function renderParcelHistorySummary(parcels) {
  ...
  return `
    <div class="parcel-history-summary-item"><span>???</span><strong>${parcels.length}</strong></div>
    <div class="parcel-history-summary-item"><span>??????????</span><strong>${delivered}</strong></div>
    <div class="parcel-history-summary-item"><span>?? ??????????</span><strong>${failed}</strong></div>
    <div class="parcel-history-summary-item"><span>???????????? ????</span><strong>${escapeHtml(formatMoney(outstandingCash))}</strong></div>
    <div class="parcel-history-summary-item"><span>???????? ????????????</span><strong>${escapeHtml(formatMoney(sumCourierPay(parcels)))}</strong></div>
  `;
}
...
function renderSelectedParcelCard() {
  ...
  const statusControls = pin.status !== "delivered" || state.isAdmin
