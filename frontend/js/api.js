...
function getLocalStaticRecoveryStore() {
  const stores = [];
  const stored = loadData(STATIC_DEPLOY_STORAGE_KEY);
  if (stored && typeof stored === "object") stores.push(stored);
  if (loadStaticBootstrap.cache && typeof loadStaticBootstrap.cache === "object") stores.push(loadStaticBootstrap.cache);

  const mergedStore = normalizeStaticStore(mergeStaticStores(...stores));
  const financeData = mergedStore.financeData && typeof mergedStore.financeData === "object" ? mergedStore.financeData : {};
  const localFinanceData = {
    cashAdjustments: loadData(CONFIG.cashAdjustmentsStorageKey),
    partnerCashAdjustments: loadData(CONFIG.partnerCashAdjustmentsStorageKey),
    payAdjustments: loadData(CONFIG.payAdjustmentsStorageKey),
    dailyBalanceLedger: loadData(CONFIG.dailyBalanceLedgerStorageKey),
  };

  mergedStore.financeData = {
    ...financeData,
    cashAdjustments: Array.isArray(localFinanceData.cashAdjustments)
      ? mergeStaticRecordsByKey(financeData.cashAdjustments, localFinanceData.cashAdjustments, getStaticAdjustmentKey)
      : financeData.cashAdjustments,
    partnerCashAdjustments: Array.isArray(localFinanceData.partnerCashAdjustments)
      ? mergeStaticRecordsByKey(financeData.partnerCashAdjustments, localFinanceData.partnerCashAdjustments, getStaticAdjustmentKey)
      : financeData.partnerCashAdjustments,
    payAdjustments: Array.isArray(localFinanceData.payAdjustments)
      ? mergeStaticRecordsByKey(financeData.payAdjustments, localFinanceData.payAdjustments, getStaticAdjustmentKey)
      : financeData.payAdjustments,
    dailyBalanceLedger: Array.isArray(localFinanceData.dailyBalanceLedger)
      ? mergeStaticRecordsByKey(financeData.dailyBalanceLedger, localFinanceData.dailyBalanceLedger, getStaticAdjustmentKey)
      : financeData.dailyBalanceLedger,
  };

  return normalizeStaticStore(mergedStore);
}

function getStaticStoreRecoverySummary(store = getLocalStaticRecoveryStore()) {
  ...
  return {
    ...
    pending: Array.isArray(store?.pending) ? store.pending.length : 0,
    parcels: Array.isArray(store?.parcels) ? store.parcels.length : 0,
    history: Array.isArray(store?.history) ? store.history.length : 0,
    zones: Array.isArray(store?.zones) ? store.zones.length : 0,
    cashAdjustments: Array.isArray(financeData.cashAdjustments) ? financeData.cashAdjustments.length : 0,
    partnerCashAdjustments: Array.isArray(financeData.partnerCashAdjustments) ? financeData.partnerCashAdjustments.length : 0,
    payAdjustments: Array.isArray(financeData.payAdjustments) ? financeData.payAdjustments.length : 0,
    dailyBalanceLedger: Array.isArray(financeData.dailyBalanceLedger) ? financeData.dailyBalanceLedger.length : 0,
  };
}

async function recoverLocalStaticStoreToFirebase() {
  if (!isStaticDeploy()) throw new Error("Recovery ხელმისაწვდომია მხოლოდ live/static აპში.");
  if (typeof saveFirebaseStaticStore !== "function") throw new Error("Firebase sync ფუნქცია ვერ მოიძებნა.");

  await loadStaticBootstrap();
  const recoveryStore = getLocalStaticRecoveryStore();
  const before = getStaticStoreRecoverySummary(recoveryStore);
  await saveFirebaseStaticStore(recoveryStore, { requireFirebase: true });
  if (loadStaticBootstrap.cache && typeof loadStaticBootstrap.cache === "object") {
    hydrateStaticFinanceStorage(loadStaticBootstrap.cache.financeData);
    saveData(STATIC_DEPLOY_STORAGE_KEY, loadStaticBootstrap.cache);
  }
  return {
    before,
    after: getStaticStoreRecoverySummary(loadStaticBootstrap.cache || recoveryStore),
  };
}
