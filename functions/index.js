...
const STICKER_ADDRESS_START_PATTERN = /(თბილისი|რუსთავი|ორთაჭალ(?:ა|აში)?|(?:დიდი|დაბალი)\s+დიღომი|დიღომი|ნუცუბიძ(?:ე|ის)?|ვაჟა[-\s]?ფშაველ(?:ა|ას)?|ვარკეთილ(?:ი|ში)?|გლდანი|ვაკე|საბურთალო|ისანი|სამგორი|ვერა|მთაწმინდა|ვაზისუბ(?:ანი|ანში|ნის|ანის)?|გულუა(?:ს)?|ლერმონტოვ(?:ი|ის)?|არჩილ\s+მეფ(?:ე|ის)?|ფარნავაზ(?:ი|ის)?|ორბელიან(?:ი|ის)?)/i;
...
function cleanStickerAddressCandidate(line) {
  ...
  value = value
    ...
    .replace(/ვაზისუბანში|ვაზისუბნის|ვაზისუბანის/gi, "ვაზისუბანი")
...
function scoreAddressLine(line) {
  ...
  if (/(კორპუსი|კორპ\.?|სართული|პლატო|კვარტალი|კვ\.?|მიკრო|მიკრორაიონი|მკრ|მ\s*\/\s*რ|შესახვევი)/i.test(value)) score += 4;
  if (/(თბილისი|რუსთავი|ორთაჭალა|გულუა|ნუცუბიძე|ნუცუბიძის|ვაჟა[-\s]?ფშაველა|ვაჟა[-\s]?ფშაველას|დიღომი|ვარკეთილი|ლერმონტოვი|ლერმონტოვის|არჩილ მეფე|არჩილ მეფის|ფარნავაზი|ფარნავაზის|ორბელიანი|ორბელიანის|ვაკე|საბურთალო|ისანი|სამგორი|გლდანი|ვერა|მთაწმინდა)/i.test(value)) score += 2;
  if (/^(?:თბილისი,\s*)?(?:ორთაჭალა|დიდი დიღომი|დაბალი დიღომი|დიღომი|ნუცუბიძე|ნუცუბიძის|ვაჟა[-\s]?ფშაველა|ვაჟა[-\s]?ფშაველას|ვარკეთილი|გლდანი|ვაკე|საბურთალო|ისანი|სამგორი)/i.test(value)) score += 8;
  if (/(?:დიდი დიღომი|დაბალი დიღომი|ნუცუბიძე|ნუცუბიძის|ვაჟა[-\s]?ფშაველა|ვაჟა[-\s]?ფშაველას|ორთაჭალა|ვარკეთილი|გლდანი|ვაკე|საბურთალო|ისანი|სამგორი).*\d/i.test(value)) score += 4;
