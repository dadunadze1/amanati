...
const STICKER_ADDRESS_START_PATTERN = /(თბილისი|რუსთავი|ორთაჭალ(?:ა|აში)?|(?:დიდი|დაბალი)\s+დიღომი|დიღომი|ნუცუბიძ(?:ე|ის)?|ვაჟა[-\s]?ფშაველ(?:ა|ას)?|ვარკეთილ(?:ი|ში)?|გლდანი|ვაკე|საბურთალო|ისანი|სამგორი|ვერა|მთაწმინდა|გულუა(?:ს)?|ლერმონტოვ(?:ი|ის)?|არჩილ\s+მეფ(?:ე|ის)?|ფარნავაზ(?:ი|ის)?|ორბელიან(?:ი|ის)?)/i;
